...
#' @param labels A vector indicating the group labels (e.g., 0 and 1) for each sample in the data, used to distinguish different groups for the statistical test.
...
#'
#'
DifferentialAnalysis <- function(data, labels, method = c("t_test", "ANOVA", "FC", "PLS-DA", "OPLS-DA", "RF_RFE", "Kruskal_Wallis", "svmrfeFeatureRanking"),
  ...
  labels <- as.numeric(as.factor(data[,2]))
  ...
  if (method == "t_test") {
    ...
    return(Kruskal_Wallis(t(data2), labels))
